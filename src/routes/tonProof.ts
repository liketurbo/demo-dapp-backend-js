...
import { DOMAIN, PAYLOAD_TTL, PROOF_TTL, SHARED_SECRET } from "../constants";
...
router.post(
  ...
  async (
    ...
    const { proof, address, network } = CheckProofPayload.parse(req.body);
    ...
    if (!signatureValid) {
      res.status(400).json({ error: "invalid payload signature" });
      return;
    }
    ...
    if (BigInt(now) > expireTime) {
      res.status(400).json({ error: "payload expired" });
      return;
    }

    // check ton proof expiration
    if (now > proof.timestamp + PROOF_TTL) {
      res.status(400).json({ error: "ton proof has been expired" });
      return;
    }
